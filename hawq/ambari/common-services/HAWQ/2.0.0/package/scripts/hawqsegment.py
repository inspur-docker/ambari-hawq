...
class HawqSegment(Script):
  ...
  def configure(self, env):
    ...
    common.update_bashrc(hawqconstants.hawq_greenplum_path_file, hawqconstants.hawq_user_bashrc_file)
  ...
  def stop(self, env):
    ...
    utils.exec_hawq_operation(hawqconstants.STOP, "{0} -a".format(hawqconstants.SEGMENT), only_if=utils.chk_hawq_process_status_cmd(
  ...
  def __init_segment():
    ...
    Execute("chown {0}:{1} {2}".format(hawqconstants.hawq_user, hawqconstants.hawq_group, os.path.dirname(params.hawq_segment_dir)),
            user=hawqconstants.root_user, timeout=hawqconstants.default_exec_timeout)

    # Initialize hawq segment
    utils.exec_hawq_operation(hawqconstants.INIT, "{0} -a -v".format(hawqconstants.SEGMENT))
  ...
  def __is_segment_initialized(self):
    ...
    """
    import params
    return os.path.exists(os.path.join(params.hawq_segment_dir, hawqconstants.postmaster_opts_filename))
