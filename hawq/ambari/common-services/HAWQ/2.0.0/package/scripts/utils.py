...
def chk_hawq_process_status_cmd(port, component_name=None):
  ...
  """
  process = "gpsyncmaster" if component_name == hawqconstants.STANDBY else "postgres"
...
def create_dir_as_hawq_user(directory):
  ...
  """
  Directory(directory, recursive=True, owner=hawqconstants.hawq_user, group=hawqconstants.hawq_group)
...
def exec_hawq_operation(operation, option, not_if=None, only_if=None, logoutput=True):
  ...
  """
  hawq_cmd = "source {0} && hawq {1} {2}".format(hawqconstants.hawq_greenplum_path_file, operation, option)
  Execute(
        hawq_cmd,
        user=hawqconstants.hawq_user,
        timeout=hawqconstants.hawq_operation_exec_timeout,
        not_if=not_if,
        only_if=only_if,
        logoutput=logoutput)
...
def exec_ssh_cmd(hostname, cmd):
  ...
  if params.hostname != hostname:
    cmd = "su - {0} -c 'ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {1} \"{2} \" '".format(hawqconstants.hawq_user, hostname, cmd)
  else:
    cmd = "su - {0} -c \"{1}\"".format(hawqconstants.hawq_user, cmd)
...
def exec_psql_cmd(command, host, db="template1", tuples_only=True):
  ...
  """
  src_cmd = "source {0}".format(hawqconstants.hawq_greenplum_path_file)
  if tuples_only:
