...
class HAWQServiceCheck(Script):
  ...
  def drop_table(self):
    Logger.info("Dropping {0} table if exists".format(hawqconstants.smoke_check_table_name))
    sql_cmd = "drop table if exists {0}".format(hawqconstants.smoke_check_table_name)
    exec_psql_cmd(sql_cmd, self.active_master_host)


  def create_table(self):
    Logger.info("Creating table {0}".format(hawqconstants.smoke_check_table_name))
    sql_cmd = "create table {0} (col1 int) distributed randomly".format(hawqconstants.smoke_check_table_name)
    exec_psql_cmd(sql_cmd, self.active_master_host)


  def insert_data(self):
    Logger.info("Inserting data to table {0}".format(hawqconstants.smoke_check_table_name))
    sql_cmd = "insert into {0} select * from generate_series(1,10)".format(hawqconstants.smoke_check_table_name)
    exec_psql_cmd(sql_cmd, self.active_master_host)


  def query_data(self):
    Logger.info("Querying data from table {0}".format(hawqconstants.smoke_check_table_name))
    sql_cmd = "select * from {0}".format(hawqconstants.smoke_check_table_name)
    exec_psql_cmd(sql_cmd, self.active_master_host)
  ...
  def check_data_correctness(self):
    ...
    sql_cmd = "select sum(col1) from {0}".format(hawqconstants.smoke_check_table_name)
  ...
  def check_state(self):
    import params
    command = "source {0} && hawq state -d {1}".format(hawqconstants.hawq_greenplum_path_file, params.hawq_master_dir)
